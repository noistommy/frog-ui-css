...
`Frog UI`는 시멘틱 클래스 기반의 스타일링 시스템입니다. 기본적으로 정의된 컴포넌트 스타일일에 유틸리티 클래스의 적용을 지양하는것을 원칙으로 합니다. 하지만 컴포넌트들의 배치, 레이아웃 조정 등 사용자 커스텀 스타일링의 경우 유틸리티 클래스를 사용하는 것이 더 유리합니다. 그래서 별도의 라이브러리를 사용하지 않아도 되도록 최소의 가장 유용한 유틸리티 클래스들을 정의 하였습니다.
...
#### Grow / Shrink (all)
`grow`: 아이템들에 균일하게 크기를 적용하여 플랙스 공간을 채웁니다. (item > flex-grow: 1) <br />
